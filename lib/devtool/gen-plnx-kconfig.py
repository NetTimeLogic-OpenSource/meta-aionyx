...
header = '# AionyxOS Auto generated Kconfig file'
kconfig_string = '''
config {0}
    bool "{1}"
    help
...
def aionyxkconfig(args, config, basepath, workspace):
    ...
    # Include aionyx-parse-recipes.bbclass into .conf file
    class_name = 'aionyx-parse-recipes'
    ...
    for line in allrecipes.output[index:].splitlines():
        ...
        if recipe_name:
            ...
            elif recipe_name.startswith('packagegroup-') and not recipe_name.startswith('packagegroup-aionyx-'):
                packages_dict['Package Groups'][recipe_name] = [ version, description, rdepends, packages ]
            elif recipe_name.startswith('packagegroup-aionyx-'):
                packages_dict['Package Groups Aionyx'][recipe_name] = [ version, description, rdepends, packages ]
            else:
...
def register_commands(subparsers, context):
    ...
    parser_systest = subparsers.add_parser('gen-aionyx-kconfig', help='Generate the aionyx rootfs configs ',
                        description='Generate the Kconfig file for all the recipes compatible to MACHINE with static DESCRIPTION/RDEPENDS/PACKAGES variables defined in recipe files. Example command: MACHINE=zynqmp-generic devtool gen-aionyx-kconfig')
    ...
    parser_systest.set_defaults(func=aionyxkconfig, no_workspace=True)
