AIONYX_XEN_DEPLOY = "${@ 'image-xlnx-xen' if d.getVar('IMAGE_AIONYX_XEN_DEPLOY') == '1' else '' }"
inherit deploy ${AIONYX_XEN_DEPLOY}
...
python() {
    ...
    xen_deploy = d.getVar('IMAGE_AIONYX_XEN_DEPLOY') or ''
    xen_sdt_flow = d.getVar('ENABLE_XEN_UBOOT_SCR') or ''
    if initramfs and xen_deploy and not xen_sdt_flow:
        d.appendVarFlag('do_deploy', 'postfuncs', ' aionyx_compile_image_builder')
}
