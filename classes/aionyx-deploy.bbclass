python () {
    if bb.data.inherits_class('image', d):
        d.appendVarFlag('do_image_complete','postfuncs', ' aionyx_deploy_rootfs')
    ...
    if pn in packagelist:
        copyfiles_update(d)
        d.appendVarFlag('do_deploy', 'postfuncs', ' aionyx_deploy')
        d.appendVarFlag('do_deploy_setscene', 'postfuncs', ' aionyx_deploy')
    if pn in mc_packagelist:
        copyfiles_update(d)
        d.appendVarFlag('do_deploy', 'postfuncs', ' mc_aionyx_deploy')
        d.appendVarFlag('do_deploy_setscene', 'postfuncs', ' mc_aionyx_deploy')
...
AIONYX_DEPLOY_DIR ?= "${TOPDIR}/images/linux"
MC_AIONYX_DEPLOY_DIR ?= "${TOPDIR}/images/${BB_CURRENT_MC}"
EXTRA_FILESLIST ?= ""
...
aionyx_deploy[dirs] ?= "${AIONYX_DEPLOY_DIR}"
python aionyx_deploy() {
    pn = d.getVar('PN')

    deploy_dir = d.getVar('DEPLOYDIR') or ""
    output_path = d.getVarFlag('aionyx_deploy', 'dirs')
...
mc_aionyx_deploy[dirs] ?= "${MC_AIONYX_DEPLOY_DIR}"
python mc_aionyx_deploy() {
    pn = d.getVar('PN')

    deploy_dir = d.getVar('DEPLOYDIR') or ""
    mc_output_path = d.getVarFlag('mc_aionyx_deploy', 'dirs')
...
aionyx_deploy_rootfs[dirs] ?= "${AIONYX_DEPLOY_DIR}"
python aionyx_deploy_rootfs() {
    import os
    import re
