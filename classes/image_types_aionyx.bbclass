...
# certain filetypes.  This allows AionyxOS to construct a single rootfs
...
#    AIONYX_IMAGE_CPIO_SKIP = "! -path './boot/*'"
# Note the matching path needs to be '.' for ${IMAGE_ROOTFS}, and any
# globbing needs to be quoted to prevent expansion.
AIONYX_IMAGE_CPIO_SKIP ?= ""
IMAGE_CMD:cpio () {
	(cd ${IMAGE_ROOTFS} && find . ${AIONYX_IMAGE_CPIO_SKIP} | sort | cpio --reproducible -o -H newc >${IMGDEPLOYDIR}/${IMAGE_NAME}.cpio)
...
# Format of this needs to be argument(s) to find, such as:
# To skip /boot directory
#    AIONYX_IMAGE_UBI_SKIP = "! -path './boot/*'"
# Note the matching path needs to be '.' for ${IMAGE_ROOTFS}, and any
# globbing needs to be quoted to prevent expansion.
# Affects ubi, ubifs and multiubi
AIONYX_IMAGE_UBI_SKIP ?= ""
...
multiubi_mkfs() {
	...
	if [ -n "$vname" ]; then
		(cd ${IMAGE_ROOTFS} && find . ${AIONYX_IMAGE_UBI_SKIP} | sort | cpio -o -H newc) | cpio -i -d -m --sparse -D ${WORKDIR}/multiubi
...
IMAGE_CMD:ubifs () {
	(cd ${IMAGE_ROOTFS} && find . ${AIONYX_IMAGE_UBI_SKIP} | sort | cpio -o -H newc) | cpio -i -d -m --sparse -D ${WORKDIR}/ubifs
