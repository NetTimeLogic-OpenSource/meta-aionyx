XEN_CONFIG ?= "${AIONYX_DEPLOY_DIR}/xen.cfg"
XEN_CONFIG_SKIP ?= "0"
MEMORY_START ?= "0x0"
...
aionyx_compile_image_builder[dirs] ?= "${WORKDIR}/image_builder"
aionyx_compile_image_builder[dirs] ?= "${AIONYX_DEPLOY_DIR}"
aionyx_compile_image_builder() {
	if [ "${XEN_CONFIG_SKIP}" != "1" ];then
		aionyx_config_image_builder
	fi
	if [ -f "${XEN_CONFIG}" ]; then
                ${DEPLOY_DIR_IMAGE}/scripts/uboot-script-gen -c ${XEN_CONFIG} -t tftp -d $(dirname "${XEN_CONFIG}") -o ${AIONYX_DEPLOY_DIR}/xen_boot_tftp
                ${DEPLOY_DIR_IMAGE}/scripts/uboot-script-gen -c ${XEN_CONFIG} -t "${LOAD_OPT}" -d $(dirname "${XEN_CONFIG}") -o ${AIONYX_DEPLOY_DIR}/xen_boot_sd
        fi
}
