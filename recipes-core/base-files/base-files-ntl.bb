AIONYX_BSP ?= "${MACHINE}"
AIONYX_PRODUCT ?= "${MACHINE}"

SUMMARY = "Aionyx identification files for the base system"
DESCRIPTION = "Identify the configuration that was used to generate this rootfs"
SECTION = "base"
PV = "${AIONYX_VERSION}"
LICENSE = "MIT"
LIC_FILES_CHKSUM = "file://${COREBASE}/meta/COPYING.MIT;md5=3da9cfbcb788c80a0384361b4de20420"

INHIBIT_DEFAULT_DEPS = "1"

# We assume the machine is generic and the swtiching is based on board or soc variants.
PACKAGE_ARCH = "${MACHINE_ARCH}"

do_configure[noexec] = '1'
do_compile[noexec] = '1'

do_install () {
        install -d -m 0755 ${D}${sysconfdir}/${DISTRO}

	echo "${AIONYX_BSP}" > ${D}${sysconfdir}/${DISTRO}/bsp
	echo "${AIONYX_PRODUCT}" > ${D}${sysconfdir}/${DISTRO}/product
	echo "${DISTRO_VERSION}" > ${D}${sysconfdir}/${DISTRO}/version
}

FILES:${PN} = "${sysconfdir}/${DISTRO}"
CONFFILES:${PN} = " \
	${sysconfdir}/${DISTRO}/bsp \
	${sysconfdir}/${DISTRO}/product \
	${sysconfdir}/${DISTRO}/version \
	"